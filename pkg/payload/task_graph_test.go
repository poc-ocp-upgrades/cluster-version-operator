...
func Test_TaskGraph_real(t *testing.T) {
	...
	for i := range p.Manifests {
		tasks = append(tasks, &Task{Manifest: &p.Manifests[i]})
	}
...
func TestRunGraph(t *testing.T) {
	...
	tests := []struct {
		...
		wantErrs	[]string
	}{{nodes: []*TaskNode{{Tasks: tasks("a", "b")}}, order: []string{"a", "b"}}, {nodes: []*TaskNode{{Tasks: tasks("c"), In: []int{3}}, {Tasks: tasks("d", "e"), In: []int{3}}, {Tasks: tasks("f"), In: []int{3}, Out: []int{4}}, {Tasks: tasks("a", "b"), Out: []int{0, 1, 2}}, {Tasks: tasks("g"), In: []int{2}}}, want: []string{"a", "b", "c", "d", "e", "f", "g"}, sleep: time.Millisecond, parallel: 2, invariants: func(t *testing.T, got []string) {
	...
	}}, {nodes: []*TaskNode{{Tasks: tasks("c"), In: []int{2}}, {Tasks: tasks("d"), In: []int{2}, Out: []int{3}}, {Tasks: tasks("a", "b"), Out: []int{0, 1}}, {Tasks: tasks("e"), In: []int{1}}}, sleep: time.Millisecond, parallel: 2, errorOn: func(t *testing.T, name string, ctx context.Context, cancelFn func()) error {
		if name == "d" {
			return fmt.Errorf("error A")
		}
		return nil
	}, want: []string{"a", "b", "c"}, wantErrs: []string{"error A"}, invariants: func(t *testing.T, got []string) {
		for _, s := range got {
			if s == "e" {
		...
		return nil
	}, want: []string{"a", "b", "d1", "d2", "d3"}, wantErrs: []string{"error - c1", "error - f"}}}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			g := &TaskGraph{Nodes: tt.nodes}
